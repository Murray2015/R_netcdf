...
Hello! This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook introducing R as a scientific computing language through climate data. When you execute code within the notebook by clicking in the grey boxes and pressing *Ctrl+Enter*, the results appear beneath the code. This document needs to be opened and run in R studio. If you open it in any other program (e.g. a web browser) you will be able to see the code, but not be able to change it, play with it or run it. You will just see the results from when I ran and compiled this document. 
