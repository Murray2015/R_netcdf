...
1. Open the NetCDF file in R with the "nc_open()" function
2. Find out what is actually inside the netcdf file with the "print()" function
3. Extract the variables we want to use from the NetCDF file using the "ncget_var()" function
4. Close the NetCDF file in R with the "nc_close()" function.
...
Ok, we do have data in the gpp variable. It's ok. 
